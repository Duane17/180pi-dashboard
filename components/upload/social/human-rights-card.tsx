    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3 sm:max-w-lg">
        <SelectField
          ...
          onChange={(v) => onChange({ policyExists: (v as "yes" | "no") || null })}
          allowEmpty
        />
      </div>

      {/* Coverage (checkbox matrix with Other + Specify) */}
      <div className="rounded-2xl border border-white/30 bg-white/50 p-4 shadow-sm backdrop-blur supports-[backdrop-filter]:bg-white/40">
        ...
        {/* Soft validation hint; Zod already enforces this constraint */}
        {otherNeedsText && (
          ...
          return (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 lg:grid-cols-7">
              ...
              <SelectField
                ...
                value={row.confirmed}
                options={YES_NO as unknown as readonly string[]}
                onChange={(v) => update({ confirmed: (v as "yes" | "no") || "no" })}
              />
              {/* Description spans the remaining width on larger screens */}
              <div className="sm:col-span-2 lg:col-span-4">
                <TextField
                    label="Short description (optional)"
                    value={row.description ?? undefined}  // matches the component's type exactly
